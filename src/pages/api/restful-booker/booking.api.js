...
export default class BookingAPIBase {
  ...
  async getAuthToken() {
    if (!this.authToken) {
      logger.step('Getting auth token');
  ...
  async createBooking(bookingData = {}) {
    logger.step('Creating new booking');
  ...
  async getBookingById(bookingId) {
    logger.step(`Getting booking by ID: ${bookingId}`);
  ...
  async getBookingsByFilter(filters = {}) {
    logger.step(`Getting bookings by filter: ${JSON.stringify(filters)}`);
  ...
  async updateBooking(bookingId, bookingData, partial = false) {
    ...
    logger.step(`Updating booking ID: ${bookingId}`);
    const payload = APIRequestHelper.getBookingPayload(bookingData);
  ...
  async partialUpdateBooking(bookingId, bookingData) {
    const token = await this.getAuthToken();
    logger.step(`Partially updating booking ID: ${bookingId}`);
    const payload = APIRequestHelper.getBookingPayload(bookingData);
  ...
  async deleteBooking(bookingId) {
    const token = await this.getAuthToken();
    logger.step(`Deleting booking ID: ${bookingId}`);
  ...
  async validateBookingResponse(booking) {
    logger.step('Validating booking response');
